...
1. **Package structure:** Using a scaffolder (`hatchit`) gives every project the same starting structure.
...
[hatchit](https://github.com/BiocPy/hatchit) (based on [hatchit](https://github.com/BiocPy/hatchit) or optionally [PyScaffold](https://pyscaffold.org/en/stable/)) automates some of the common configurtion we use across all BiocPy packages. Yes, it's a package to create packages—very meta I know :).

First, install `hatchit` if you haven't already (`pip install hatchit`).

Then, run the `hatchit` command:

```sh
⋊> ~/P/scratch hatchit my-awesome-package
hatchit complete! 🚀 💥
```
...
├── pyproject.toml    # Modern Python project metadata & build system config (hatch and uv)
...
- **Running tests:** We use [`tox`](https://tox.wiki/en/4.26.0/) powered by `uv`. These tools create a temporary virtual environment, install your package and its dependencies _exactly_ as defined, and then run `pytest`. This ensures your tests run in a clean, reproducible environment, mimicking how users will install your package. Run `tox` to run the tests.
...
default: commands[0]> uv run pytest
..............
..............
..............
collected 2 items
...
While you're editing `pyproject.toml`, also explore the package metadata, especially description, links to documentation, repository information etc.
...
- **Building the docs:** Use `tox -e docs` (or `tox -e docs` if using PyScaffold)\! This ensures the docs build in a clean environment with all necessary extensions.
...
3. **Test locally:** Run `tox` (or `tox`) frequently\! Catch errors early.
...
6. **CI checks:** GitHub Actions (configured in `.github/workflows/run-tests.yml`) will automatically run `tox` (or `tox`) on your PR to ensure tests pass on different Python versions and platforms (windows, mac and linux).
...
The scaffolding uses `hatch-vcs` to manage your version numbers automatically based on git tags. This is incredibly handy because:
...
And that's the gist of it\! It might seem like a lot initially, but once you go through the cycle a couple of times, it becomes second nature. Remember the key steps: scaffold, code in `src/`, test in `tests/` (run with `tox`), manage dependencies, document in `docs/` (build with `tox -e docs`), use branches for development, and tag for releases.
